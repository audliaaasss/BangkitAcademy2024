...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private val resultLauncher = registerForActivityResult(
        ...
        if (result.resultCode == FormUserPreferenceActivity.RESULT_CODE && result.data != null) {
            val userModel = if (Build.VERSION.SDK_INT >= 33) {
                result.data?.getParcelableExtra(FormUserPreferenceActivity.EXTRA_RESULT, UserModel::class.java)
            } else {
                @Suppress ("DEPRECATION")
                result.data?.getParcelableExtra<UserModel>(FormUserPreferenceActivity.EXTRA_RESULT)
